...
class CaveCarver {
public:
	// Coarse-lattice samples of the 3D fields, covering one chunk.
	struct Grid {
	...
	struct Column {
		double region = 0.; // 0 = no caves, 1 = full strength
		int spaghettiCover = 0; // solid blocks tunnels must stay under
		int lavaLevel = 0; // carved cells below this fill with lava (one flat level per region)
		int wallLevel = 0; // on a region edge, keep rock solid below this so heights don't merge
		bool ravine = false;
		int ravineTop = 0;
	...
	// True if the solid block at pos should be carved to air. 'local' indexes the chunk (for lattice
	// lookup), 'depth' is the solid blocks above it in the column.
	bool isCarved(const Grid& grid, const Column& col, ivec3 local, int depth) const;

	// Like isCarved but samples the fields straight at the point, for one-off lookups (a structure's
	// support height) where building a whole grid would be wasteful.
...
private:
	...
	// Above this, drop the cover so tunnels break out into surface mouths.
	static constexpr double REGION_OPEN = 0.7;

	// Solid blocks of cover before carving. Thin tubes stay buried so they don't scar the grass;
	// only big caverns and ravines break into mouths.
	static constexpr int SUBMERGED_COVER = 6;          // seal the seabed
	// Thin tubes stay buried on flat land but open right onto mountain cliffs.
	...
	// Lava lakes: warped cells, each pooling at one flat level so a pool never slopes. A rock band
	// (LAKE_WALL) stays solid below the higher level where two heights meet.
	...
	// Drives cliff openings and guarantees caves even where the region mask is low.
	...
	// Interpolates a field over the world lattice, same as isCarved but without a pre-built grid.
