...
CaveCarver::Column CaveCarver::column(ivec2 xz, int surfaceHeight) const {
	...
	// Dense clusters open mouths to the surface instead of staying sealed under the cover.
...
void CaveCarver::lakeAt(ivec2 xz, Column& col) const {
	// Warp the cell grid so region edges curve; sample once and reuse for the seam lookups.
	dvec2 p(xz);
	...
	// Look a wall's width out in eight directions; on a seam, keep solid to the higher level.
	int hi = level;
	bool seam = false;
...
// Shared carving decision; each caller passes how a field is sampled. A sampler only runs when its
// cover lets it carve, so a field is never sampled when it can't change the result.
