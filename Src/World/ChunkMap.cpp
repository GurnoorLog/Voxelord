...
void ChunkMap::applyEdits(const std::vector<BlockEdit>& edits) {
	...
	// Group the edits by chunk, resolving each chunk once under the lock and holding a shared_ptr
	// so the orchestrator can't free it mid-write.
	...
	// Light can touch the edited chunk plus its eight surrounding neighbors.
	...
	// Fix up the light around the edits and add the changed sections to the remesh set below.
	...
	// Remesh every section a change can affect: each edited section and its six face neighbors
	// (a face is hidden by the block on the other side). May rebuild a neighbor that didn't
	// change, but that's just a wasted rebuild, never a stale mesh.
	...
	// Light can change well past the edited block (removing a torch darkens its whole radius).
...
bool ChunkMap::submitBulkEdit(EditGenerator generator) {
	std::lock_guard<std::mutex> lock(m_chunksMutex);
	// Only one pending or running, so two never remesh the same section at once.
...
void ChunkMap::doLoadBlocks(ivec2 pos) {
	std::shared_ptr<Chunk> chunk;
	{
		...
		if (!isInLoadDistance(pos)) // moved out of range while queued
			return;
		auto it = m_chunks.find(pos);
		if (it == m_chunks.end()) // not in the map yet
	...
	// CAS so two workers never generate the same chunk.
	if (!chunk->casState(Chunk::TO_LOAD_BLOCKS, Chunk::LOADING_BLOCKS))
		return;
	// Generating only mutates this chunk, so it runs without the lock.
	chunk->loadBlocks();
	// Blocks just appeared; re-check the 3x3 since any neighbor may now be mesh-ready.
	std::lock_guard<std::mutex> lock(m_chunksMutex);
	for (ivec2 off : NEIGHBORHOOD_3X3)
...
void ChunkMap::doLoadMesh(ivec2 pos) {
	...
	NeighborChunks neighbors{};
	{
		...
		if (it == m_chunks.end())
			return;
		// Snapshot the 3x3 neighborhood (center included) so the build never reads the map.
		for (ivec2 off : NEIGHBORHOOD_3X3) {
	...
	// Before meshing, settle light across the borders; the neighbors are at least TO_LOAD_MESH
	// so their light is ready. The lock stops two spills racing on the same light bytes.
	...
	// The main thread uploads the VAOs in update().
	{
		std::lock_guard<std::mutex> lock(m_chunksMutex);
		m_chunksToUploadMesh.push(pos);
		m_chunksPendingUpload.insert(pos);
	}
	// Rebuild the sections the spill brightened. Runs after the publish so it can't race this chunk's
	// upload. A neighbor not yet meshed picks up the spilled light on its own; one meshing right now
	// may show a faint seam until its next rebuild, which any later edit fixes.
	remeshSections(lightDirtySections);
}

void ChunkMap::doBulkEdit(EditGenerator generator) {
	// Compute off the lock, then apply + remesh through the same path a single edit uses.
...
void ChunkMap::remeshSections(const std::vector<ivec3>& sections) {
	...
	// Snapshot under the lock which sections still exist on a rendered chunk, plus the neighbor
	// chunks their border faces read from.
	...
	// Publish the GPU uploads to the main thread.
	std::lock_guard<std::mutex> lock(m_chunksMutex);
	for (MeshJob& job : jobs) {
		// If the chunk still owes a full upload, that upload already carries this section; a section
		// upload would push an already-consumed empty mesh.
...
bool ChunkMap::canChunkBeEdited(ivec2 pos) const {
	// A remesh reads the full 3x3 neighborhood, so all must exist.
	for (ivec2 off : NEIGHBORHOOD_3X3)
...
void ChunkMap::enqueueMeshIfReady(ivec2 pos) {
	...
	// All eight neighbors' blocks must be ready: corner lighting samples the diagonal chunks.
	for (ivec2 off : NEIGHBORHOOD_3X3) {
		auto neighborIt = m_chunks.find(pos + off);
