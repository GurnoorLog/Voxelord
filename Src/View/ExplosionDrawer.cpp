...
namespace {
	...
	// Build a unit sphere; each vertex doubles as an outward direction the shader uses for noise.
...
void ExplosionDrawer::spawn(vec3 center, float radius) {
	...
	e.age = 0.f;
	// Bigger blasts last longer (mostly proportional, so a small one stays snappy).
	e.duration = 0.22f + 0.019f * radius;
	e.density = 8.f + 0.14f * radius; // bigger = denser, more opaque
	// 0 for a small blast, 1 for huge; the shader turns a sharp burst into a slow rolling fireball.
...
void ExplosionDrawer::render(const mat4& view, const mat4& projection, const vec4& clipPlane, const vec3& skyColor) {
	...
	// Remember the GL state we change so we can put it back.
	...
	// Depth-test against the world so terrain hides the fireball, but don't write depth: the shells
	// overlap and shouldn't block each other. Culling off so both sides of each shell get drawn.
	...
	glEnable(GL_BLEND);
	// Premultiplied "over" blending: each shell glows and covers what's behind it, so the fireball
	// reads as a solid mass rather than a see-through glow.
	...
	for (const Explosion& e : m_explosions) {
		...
		// Draw shells outer to inner so the bright core layers on top of the dimmer haze.
