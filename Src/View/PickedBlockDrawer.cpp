...
DefaultMesh PickedBlockDrawer::buildHeldBlockMesh(Block block) {
	...
	for (Dir3D::Dir dir : Dir3D::all()) {
		...
		for (int vtx = 0; vtx < 4; ++vtx) {
			...
			// Full block light so the preview stays bright regardless of day/night.
...
void PickedBlockDrawer::render(std::optional<Block> picked, Window* p_window, DefaultRenderer& renderer) {
	...
	// Mesh only depends on the block type, so rebuild it just when the pick changes.
	...
	// Small square in the bottom-left corner on a freshly cleared depth buffer, so the cube sits on
	// top of the scene yet still self-occludes correctly.
