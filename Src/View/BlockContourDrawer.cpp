...
void BlockContourDrawer::render(std::optional<ivec3> target, const mat4& view, const mat4& projection) {
	...
	// Depth-test against the world so the outline is occluded by closer geometry and its back edges
	// by the targeted block; the shader's slight inflation keeps front edges off the block's faces.
