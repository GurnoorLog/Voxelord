...
void WindowTextDrawer::drawChat(Game& game) {
	sf::Vector2u win = p_window->getSize();
	unsigned int charSize = static_cast<unsigned int>(textSize * win.y);
	const float spacing = textSpacing * 0.9f;

	float cursorY = 1.f - spacing;
	if (game.isChatOpen()) {
		sf::Text input;
		input.setFont(m_font);
		input.setString("> " + game.getChatBuffer() + "_");
		input.setCharacterSize(charSize);
		input.setFillColor(sf::Color{ 255, 255, 120 });
		input.setPosition({ paddingLeft * win.x, win.y * cursorY });
		p_window->draw(input);
		cursorY -= spacing;
	}

	const auto& lines = game.getChatLines();
	for (std::size_t k = 0; k < lines.size(); ++k) {
		const ChatLine& line = lines[lines.size() - 1 - k];
		sf::Text text;
		text.setFont(m_font);
		text.setString("<" + line.sender + "> " + line.text);
		text.setCharacterSize(charSize);
		text.setFillColor(sf::Color::White);
		text.setPosition({ paddingLeft * win.x, win.y * (cursorY - static_cast<float>(k) * spacing) });
		p_window->draw(text);
	}
}
